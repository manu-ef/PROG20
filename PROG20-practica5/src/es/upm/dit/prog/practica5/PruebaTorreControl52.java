...
public class PruebaTorreControl52 { //implements ActionListener {
...
//	private static final int N_AES = 10;
    ...
    public void step() {
		...
		List<Aeronave> aes = this.tc.getAeronaves(new SelectorAeronaveTrue());
		for (Aeronave a: aes)
			a.mover(time);
		...
		if (this.selectorAmenazadas) {
			...
			for (Aeronave a: aes) {
				for (Aeronave b: this.tc.getAeronaves(new SelectorAeronaveAmenazaA(a)))
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                PruebaTorreControl52 pva = new PruebaTorreControl52();
                pva.init();
    ...
    private void createAndShowGUI() {
    	...
    	this.rp.setAeronaves(this.tc.getAeronaves(new SelectorAeronaveTrue()));
