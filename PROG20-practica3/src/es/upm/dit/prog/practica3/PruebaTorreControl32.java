...
public class PruebaTorreControl32 { //implements ActionListener {
    ...
    public void step() {
		...
		for (Aeronave a: Arrays.asList(this.tc.getAeronaves()))
			a.mover(time);
		...
		if (this.selectorTrue)
			this.rp.setAeronaves(Arrays.asList(this.tc.getAeronaves()));
		if (this.selectorAmenazadas) {
			this.rp.setEnPeligro(true);
			this.rp.setAeronaves(Arrays.asList(this.tc.getAmenazadas()));
//			List<Line2D> lineas = new ArrayList<Line2D>();
//			for (Aeronave a: Arrays.asList(this.tc.getAeronaves())) {
//				for (Aeronave b: Arrays.asList(this.tc.getAmenazasA(a)))
...
//		}
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                PruebaTorreControl32 pva = new PruebaTorreControl32();
                pva.init();
